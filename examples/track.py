...
@torch.no_grad()
def run(args):
    print(args.yolo_model)
    print(str(args.yolo_model)=='yolov8n-pose')
    yolo = YOLO(
        args.yolo_model if 'yolov8' in str(args.yolo_model) else 'yolov8n.pt',
    )
    
    print(f'Default Video Frame Rate: {str(args.vid_stride)}')
    results = yolo.track(
        source=args.source,
        conf=args.conf,
        iou=args.iou,
    ...
    for frame_idx, r in enumerate(results):
        if r.boxes.data.shape[1] == 7:
            #if str(args.yolo_model) == 'yolov8n-pose':
            #    print(f'Keypoints of Object-{r.boxes.id} in frame-{frame_idx}: {r.keypoints[0]}')
            #print(f'Bounding Box Information for frame {frame_idx}')
            #print(f'boxes_id = {r.boxes.id}')
            #print(f'boxex_conf = {r.boxes.conf}')
            #print(f'boxes_cls = {r.boxes.cls}')
            #print(f'boxes_xyxy = {r.boxes.xyxy}')
            if yolo.predictor.source_type.webcam or args.source.endswith(VID_FORMATS):
                input_filename = str(args.source).split('/')[-1].split('.')[0]
                print(f'filename = {input_filename}')
                filename = f'{str(args.yolo_model)}_{input_filename}_vid_stride_args_{str(args.vid_stride)}_post_processed_results' 
                if str(args.yolo_model) =='yolov8n-pose':
                    filename += '_pose'

                p = yolo.predictor.save_dir / 'mot' / (filename + '.csv')
